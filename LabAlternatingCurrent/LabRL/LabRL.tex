...
\keywords{Змінний струм, $RL$-коло, активний опір, індуктивний опір,імпеданс.}
\abstract{Визначення імпедансу котушки індуктивності як функції частоти. Визначення фазового зсуву між напругою на котушці індуктивності та струмом в колі як функції частоти.}
\apparatus{генератор сигналів низькочастотний ГЗ-112, диференційний підсилювач, осцилограф C1-93, набір резисторів та котушок індуктивності.}
\chapter{Котушка індуктивності у колі змінного струму}
\makeworktitle
...
%\end{tikzpicture}
%\caption{Елеиенти керування генератором ГЗ-112}
%\label{fig:gz112}
%\end{figure}
%
...
%=========================================================
\begin{figure}[h!]\centering
        \input{\currfiledir/RL_circuit.tikz}
...
\begin{enumerate}
    ...
    \item Зберіть схему, як показано на рис.~\ref{pic:RL_circuit}. Частоту генератора \tcbox{\sc{ГЗ-112/1}} змінюйте за допомогою ручки~\circled{\ref{btn:Hz}}.
...
\begin{subfigure}[t]{0.7\linewidth}\centering
    \input{\currfiledir/XLnu.tikz}
\caption{$X_L = f(f)$}
\label{fig:X_Lnu}
\end{subfigure}
\\
\begin{subfigure}{0.7\linewidth}\centering
    \input{\currfiledir/XLL.tikz}
\caption{$X_L = f(L)$}
\label{fig:X_LL}
\end{subfigure}
...
\begin{figure}[h!]\centering
    \input{\currfiledir/tanphinuL.tikz}
